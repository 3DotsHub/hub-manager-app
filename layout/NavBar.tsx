				...
				<div className="relative w-full pt-8">
					<ul className="flex-col hidden flex-1 mr-1 gap-2 md:flex lg:gap-[2rem] scroll-auto">
						<Link className="ml-3" href={'/'}>
							<picture>
								<img className="-my-5 rounded-3xl" src="/3dotshub.png" alt="Logo" width={80} />
							</picture>
						</Link>
						<NavElementTree />
					...
					<aside className="flex w-10 h-10 sm:hidden">
						<div className="flex items-center md:block">
							<label className="absolute z-20 cursor-pointer px-3 py-6 right-0 md:right-4" htmlFor="ss-mobile-menu">
								...
								<div
									className={`fixed inset-0 h-screen w-full bg-gray-800/50 backdrop-blur-sm ${
										isOpen ? 'block' : 'hidden'
									}`}
									onClick={() => setState(false)}
								></div>
								<div
									className={`fixed top-0 right-0 h-screen w-64 translate-x-full overflow-y-auto overscroll-y-none transition duration-300 ${
										isOpen ? 'translate-x-0' : 'translate-x-100'
									}`}
								>
									<div className="min-h-full w-full bg-navbar-primary opacity-90 backdrop-blur px-6 pt-12 shadow-2xl relative">
										...
										<menu className="mt-8 pb-8 flex flex-col gap-8 text-heading">
											<NavElementTree />
										</menu>
									</div>
								</div>
